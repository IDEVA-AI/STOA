...
export function createCourse(title: string, description: string, thumbnail: string): number {
  const result = db
...
export function deleteCourse(id: number): boolean {
  const del = db.transaction(() => {
    // Delete lessons belonging to modules of this course
    db.prepare(
...
export function getLessonsByModule(moduleId: number): LessonRow[] {
  return db
    .prepare(
      `SELECT id, module_id, title, content_url, content_type, duration, "order" FROM lessons WHERE module_id = ? ORDER BY "order"`
    )
